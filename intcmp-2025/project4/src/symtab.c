...
/* push_scope: new scope entry -> call */
void push_scope() {
    scope_markers[scope_level++] = def_stack_top;
}

/* pop_scope: return symbols if current scope ends */
struct ste* pop_scope() {
    ...
    while (def_stack_top != old_marker) {
        ...
        def_stack_top = def_stack_top->prev;  /* pop */
    ...
    /* ppt logic: scope_level decrease */
    scope_level--;
...
/* declare_struct: struct는 항상 전역 scope에 등록 */
struct ste* declare_struct(struct id *name, struct decl *decl) {
    if (scope_level == 0) {
        /* global scope면 그냥 declare */
        return declare(name, decl);
    } else {
        /* struct는 항상 global scope(바닥)에 등록 */
        struct ste *new_ste = (struct ste*)malloc(sizeof(struct ste));
        new_ste->name = name;
        new_ste->decl = decl;
        
        /* 스택의 바닥까지 찾아가서 삽입 */
        struct ste *e = def_stack_top;
        if (e == NULL) {
            /* 스택이 비어있으면 새 노드가 top */
            new_ste->prev = NULL;
            def_stack_top = new_ste;
...
/* findstructdecl: struct는 항상 global scope에서 search */
struct decl* findstructdecl(struct id *name) {
    /* global scope의 끝까지 검색 */
    for (struct ste *e = def_stack_top; e != NULL; e = e->prev) {
        if (e->name == name && e->decl && 
            e->decl->declclass == DECL_TYPE && 
            e->decl->typeclass == TYPE_STRUCT) {
            return e->decl;
        }
    }
    return NULL;
...
/* check_is_type: TYPE declaration인지 check */
...
struct decl* makeconstdecl(struct decl *type) {
    ...
    d->type = type;
    d->size = (type != NULL) ? type->size : 0;  /* [Project 4] size 복사 */
    return d;
}
...
/* size_in_words: decl의 크기를 word 단위로 반환
 * 현재 d->size는 bytes 단위 (int=4, char=4, ptr=4)
 * VM은 4 bytes = 1 word
 */
...
void init_type() {
    ...
    /* [Project 4] built-in 함수 등록 */
    struct decl *func;
    struct decl *param;
    struct decl *charptr;

    /* char* 타입 (write_string용) */
    charptr = makeptrdecl(chartype);

    /* read_int(): int */
    func = makefuncdecl();
    func->returntype = inttype;
    func->formals = NULL;  /* 인자 없음 */
    declare(enter(ID, "read_int", 8), func);

    /* read_char(): char */
    func = makefuncdecl();
    func->returntype = chartype;
    func->formals = NULL;
    declare(enter(ID, "read_char", 9), func);

    /* write_int(int n): void */
    func = makefuncdecl();
    func->returntype = voidtype;
    param = makevardecl(inttype);
    push_scope();
    declare(enter(ID, "n", 1), param);
    func->formals = pop_scope();
    declare(enter(ID, "write_int", 9), func);

    /* write_char(char c): void */
    func = makefuncdecl();
    func->returntype = voidtype;
    param = makevardecl(chartype);
    push_scope();
    declare(enter(ID, "c", 1), param);
    func->formals = pop_scope();
    declare(enter(ID, "write_char", 10), func);

    /* write_string(char* s): void */
    func = makefuncdecl();
    func->returntype = voidtype;
    param = makevardecl(charptr);
    push_scope();
    declare(enter(ID, "s", 1), param);
    func->formals = pop_scope();
    declare(enter(ID, "write_string", 12), func);
}
